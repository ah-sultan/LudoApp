                                        ...
                                        <ul className={`trns-1 invisible lg:visible h-0 lg:h-auto overflow-hidden origin-top translate-y-0 lg:transform-none ${clicked === id ? "showSubNav" : undefined}`}>
                                            {
                                                navlist.navlink.map(([title, url], id) => {
                                                    return (
                                                        <li key={id} className="border-tGreay-100 border-b lg:border-0"><a href={url} className="py-2 lg:py-0 text-sm inline-block leading-[30px] text-dark-700 lg:text-black font-normal trns-1 duration-500 hover:text-primary-900 lg:hover:pl-2.5">{title}</a></li>
                                                    )
                                                })
                                            }
                                        </ul>
