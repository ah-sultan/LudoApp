            ...
            <ul className="mr-5">
                {
                    list.map(([title, url], index) => {
                        return (
                            <li key={index}><Link href={url} className="block text-sm text-dark-500 md:text-black py-3 pl-5 border-b md:uppercase border-solid border-[#eee] trns-1 md:hover:pl-25px hover:text-primary-900">{title}</Link></li>
                        )
                    })
                }
